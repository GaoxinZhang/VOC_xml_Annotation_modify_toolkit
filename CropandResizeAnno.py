...
def CropandResizeAnno(xml_name):
    ...
    for obj in object:
        ...
        cropImg = img_crop_resize(image, obj, resize_width, resize_height)
        edge_detection(cropImg,img_full_name)
...
def edge_detection(image,img_full_name):
    ...
    (shotname, extension) = os.path.splitext(img_full_name)
    rand = str(random.randint(0,999))
    crop_img_name = shotname + '_' + str(int(mean)) + '_' + rand + '.jpg'
    crop_img_file_path = os.path.join(save_img_path, crop_img_name)
    cv.imwrite(crop_img_file_path, image)
    print mean
...
def img_crop_resize(image,object,resize_width,resize_height):
    ...
    # cropImg = cv.resize(cropImg, (resize_width, resize_height))
    return cropImg

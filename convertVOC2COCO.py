...
def generateVOC2Json(rootDir, xmlFiles):
    ...
    # images = dict()
    # images1 = list()
    attrDict["categories"] = [{"supercategory": "none", "id": 1, "name": "Flake_off"},
                              {"supercategory": "none", "id": 2, "name": "Injured_fastener"},
                              {"supercategory": "none", "id": 3, "name": "Puncture_Perforation"},
                              {"supercategory": "none", "id": 4, "name": "Fastener_hole"},
                              {"supercategory": "none", "id": 5, "name": "Dent"},
                              {"supercategory": "none", "id": 6, "name": "Crack"},
                              {"supercategory": "none", "id": 7, "name": "Scratch"},
                              {"supercategory": "none", "id": 8, "name": "Corrosion"},
                              {"supercategory": "none", "id": 9, "name": "Lightening_strike"}
                              ]
    # attrDict["categories"] = [
    #                           {"supercategory": "none", "id": 1, "name": "Puncture_Perforation"}
    #                           ]
    images = list()
    annotations = list()
    for root, dirs, files in os.walk(rootDir):
        ...
        for file in xmlFiles:
            ...
            if file in files:
                ...
                image = dict()
                # keyList = list()
                doc = xmltodict.parse(open(annotation_path).read(),force_list=('b',))
                # print doc['annotation']['filename']
                # image['file_name'] = str(doc['annotation']['filename'])
                image['file_name'] = str(jpg_full_name)
                ...
                if 'object' in doc['annotation']:
                    for obj in guaranteed_list(doc['annotation']['object']):
                        # print(obj)
    ...
    with open("instances_minival2014.json", "w") as f:
        f.write(jsonString)
...
trainFile = "/home/zhanggx/coco_format_data/AIRPLANE_9/data/coco/ImageSets/Main/val.txt"
trainXMLFiles = list()
...
rootDir = "/home/zhanggx/coco_format_data/AIRPLANE_9/data/coco/VOC_annotations"
